...
namespace secure
{
    namespace syscalls
    {
        ...
        {
            auto Ldr = (_PEB_LDR_DATA2_64_T*)(((_PEB64*)((_TEB_T<std::uint64_t>*)NtCurrentTeb())->ProcessEnvironmentBlock)->Ldr);
            auto Head = (PLIST_ENTRY)&Ldr->InLoadOrderModuleList;
            auto Next = Head->Flink;

            while (Next != Head) {
                auto ent = CONTAINING_RECORD(Next, LDR_DATA_TABLE_ENTRY, InLoadOrderLinks);
                Next = Next->Flink;
